...
private:
    ...
    void dirty_init()
    {
        ...
        //  qDebug() << "libusb_release_interface (0) " << libusb_error_name( libusb_release_interface(handle, 0) ) << "\r";
        //    qDebug() << "libusb_release_interface (1) " << libusb_error_name( libusb_release_interface(handle, 1) ) << "\r";
    }
public slots:
    void start()
    {
...

{
        ...
        int rcc = libusb_set_configuration(devh, config);
        if(rcc == 0)
            qDebug() << "Succeful set configuration.\n";
        else
            qDebug() << "NOT set configuration: " << libusb_error_name( rcc ) << "\n";
        qDebug() << "libusb_claim_interface" << libusb_error_name( libusb_claim_interface(devh, iface) ) << "\n";
        int rc = libusb_set_interface_alt_setting(devh, iface, alt_config);
        if(rc == 0)
            qDebug() << "Succeful  set ALT configuration.\n";
        else
            qDebug() << "NOT set ALT configuration: " << libusb_error_name( rc ) << "\n";
        alloc_transfers();
        emit init_completed();
    }
...
public:
    ...
    libusb_async_reader()
    {
        ...
        connect(asyncc, &async::readyRead_barcode,  this, &libusb_async_reader::readyRead_inner, Qt::DirectConnection );
...
public slots:
    ...
    {
        //qDebug() << "size: " << data.size() << " / data: " << data << " / data: " << data.toHex(':');
        emit readyRead(data);
    }
